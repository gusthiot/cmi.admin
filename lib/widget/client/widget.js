...
Widget.declareClass("Select")
    .helpers({
        ...
        attributesOfEmptyOption: function () {
            ...
            var emptyIsSelected = (_.indexOf(data.widget.values, data.widget.value) == -1);
            var emptyIsDisabled = (_.indexOf(data.widget.values, undefined) == -1 );
            var attributes = {};
            if (emptyIsSelected) {
...
function dateFormatExpectedByDatepicker() {
    return 'MM-DD-YYYY';
}
...
Widget.declareClass("Date")
    .helpers({
        asDatePickerTime: function (time) {
            return moment.utc(time, dateFormatExpectedByDatepicker()).format(dateFormatExpectedByDatepicker());
        },
        formattedDate: function () {
            return (moment(Template.currentData().startTime, dateFormatExpectedByDatepicker()).format(dateFormatExpectedByDatepicker()));
        },
        dateToLocalizedString: function(t) {
            ...
            return m.locale(lang).format("l LTS");
        }
    }).viewmodel({
      value: ''
    });
...
Widget.declareClass("Time")
    .helpers({
        asPickerTime: function () {
            var time = "October 13, 2014 11:13:00";
            return moment.utc(time, timeFormatExpectedByTimepicker()).format(timeFormatExpectedByTimepicker());
        },
        formattedTime: function () {
            return (moment(Template.currentData().startTime, timeFormatExpectedByTimepicker()).format(timeFormatExpectedByTimepicker()));
        },
        timeToLocalizedString: function() {
            var t = "October 13, 2014 11:13:00";
            var m = moment(new Date(t));
            if (m === undefined) {
                return undefined;
            }
