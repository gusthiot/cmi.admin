...
/**
 * Normalizing function for Widget$Date value
 * @param {Date|moment} dateOrMoment
 * @returns A UTC time where hours, minutes and seconds are all zero
 */
function pureDayMoment(dateOrMoment) {
    if (! dateOrMoment) {
    ...
    return moment.utc({
        year:   dateOrMoment.year(),
        month:  dateOrMoment.month(),
        date:   dateOrMoment.date(),
    });
}
...
Widget.declareClass("Date")
    .helpers({
        ...
        value: ViewModel.property.object.convertIn(pureDayMoment),
    });

Template.Widget$Date$editable.onRendered(function () {
    var self = this;
    self.changed = function() {
        var newDate = self.$("input.datepicker").datepicker("getDate");
        if (! newDate) return;

        var newMoment = pureDayMoment(newDate),
          oldMoment = self.viewmodel.value.value;  // Non-reactive read
          if (! newMoment.isSame(oldMoment)) {
              self.viewmodel.value(newMoment);     // Reactive write
          }
    };
...
Widget.declareClass("Time")
    .helpers({
        toTimepickerInputValue: function () {
            ...
            return moment(this.value).format(timeFormatExpectedByTimepicker());
        },
