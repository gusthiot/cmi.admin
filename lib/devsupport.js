...
if (Meteor.isServer) {
  ...
  Debug = function(channel) {
    return function() {
      var channels;
      Tracker.nonreactive(function() { channels = Session.get("debugChannels") || {} });
      if (channels && channels[channel]) {
        console.log.apply(console, Array.prototype.slice.call(null, arguments));
      }
    }
  };

  // To call from the browser's JavaScript console
  Debug.enable = function(channel) {
    var channels;
    Tracker.nonreactive(function() { channels = Session.get("debugChannels") || {} });
    channels[channel] = true;
    Session.set("debugChannels", channels);
  };

  Debug.disable = function(opt_channel) {
    if (opt_channel === undefined) {
      Session.set("debugChannels", {});
    } else {
      var channels;
      Tracker.nonreactive(function() { channels = Session.get("debugChannels") || {} });
...
if (Meteor.isClient) {
  Meteor.startup( function () {
    // http://stackoverflow.com/questions/498469/jquery-assertion-support-defensive-programming
